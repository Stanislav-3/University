...
def des_encrypt(key, msg, pad_block_size=8):
    ...
    msg = cipher.encrypt(pad(msg.encode(), pad_block_size))
    return msg
...
def prepare_des3_key(key, pad_key_size=16):
    return pad(key.encode(), pad_key_size)


def prepare_iv(pad_block_size=8):
    return Random.new().read(pad_block_size)