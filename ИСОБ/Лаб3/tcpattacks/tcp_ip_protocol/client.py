...
class Client:
    ...
    def connect(self, to_ip, to_port, from_port=None):
        ...
        print(f'{self.to_str(from_port)}: {result_str} connected to server with ip={to_ip} port={to_port}')
        return result

    def successfully_connect_of_client(self, from_ip, from_port, to_port):
        print(f'{self.to_str(to_port)}: successfully connected to client with ip={from_ip} port={from_port}')
