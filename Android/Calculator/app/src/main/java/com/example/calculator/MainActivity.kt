...
class MainActivity : AppCompatActivity() {
    ...
    private fun appendToExpression(expressionPart: ExpressionPart) {
        val current = expressionPart.getInternalString()
        val previous = expression.content.lastOrNull()

        if (previous != null) {
            if (current == "pi" || current == "e" || current == "abs(" || current == "ln("
                || current == "lg(" || current == "sin(" || current == "cos("
                || current == "tan(" || current == "cot(" || current == "arcsin("
                || current == "arccos(" || current == "arctan(" || current == "arccot(") {
                if (previous is NumberPart
                    || previous.getInternalString() == ")"
                    || previous.getInternalString() == "pi"
                    || previous.getInternalString() == "e") {
                    expression.append(OperationSign("×","*"))
                }
            }

            if(expressionPart is NumberPart) {
                if (previous.getInternalString() == ")"
                    || previous.getInternalString() == "pi"
                    || previous.getInternalString() == "e") {
                    expression.append(OperationSign("×","*"))
                }
        ...
        if (expressionResult.isValid) {
            when {
                ...
                expressionResult.value == Double.NEGATIVE_INFINITY -> {
                    ...
                    var result = (expressionResult.value.toBeautyString().replace('.', ','))
                    if (hard) {
                        binding.textResult.text = ""
                        binding.textExpression.text = expression.resultToExpression()
                    } else {
