...
class Hacker:
    ...
    def __str__(self):
        return f'Hacker ip={self._hacker_client.ip} port={self._hacker_client.ports[0]}: ' \
...
def main():
    ...
    client.send(server_ip, server_port, 'Hi')
    client.send(server_ip, server_port, 'My name\'s Client')
    hacker.send_rst_flag(server_ip, server_port)
    client.send(server_ip, server_port, 'What\'s your name?')
