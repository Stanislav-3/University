...
class Hacker:
    ...
    def check_ports(self, ip, lport, rport):
        print(f'{self}: start checking ports from {lport} to {rport} with ip={ip}')
    ...
    def get(self, tcp_client, datagram, from_ip):
        ...
        print(f'{self}: server found at ip={from_ip} port={from_port}')
...
def main():
    ...
    server = Client(server_ip, (7969, 8005, 8024, 8042, 8070, 8096), is_server=True)
    register(server)
