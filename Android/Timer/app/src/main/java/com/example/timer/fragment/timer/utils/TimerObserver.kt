        ...
        override fun onReceive(context: Context?, intent: Intent) {
            if (intent.action == TimerService.TICK) {
                ...
                if (intent.getBooleanExtra("LAST_TICKS", false)) {
                    if (voice_on) {
                        tts!!.speak(_remainingTime.value.toString(), TextToSpeech.QUEUE_ADD, null, null)
                    } else {
                        player.start()
                    }
                }
...
//                    mService.myInit(timerList, voice_on)
                    ...
                    if (title.toString() != prev_title) {
                        prev_title = title.toString()
//                        Log.d("*** TimerObserver ***", "TITLE = ${title}")
