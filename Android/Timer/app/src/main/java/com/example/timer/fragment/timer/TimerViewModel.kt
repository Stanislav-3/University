...
class TimerViewModel(
    ...
    firstInit: Boolean,
    private val tts: TextToSpeech?): AndroidViewModel(application) {
    ...
    init{
        ...
        timerObserver = TimerObserver(timerList, context, sequence, firstInit0, tts)
        firstInit0 = false;
...
class TimerViewModelFactory(
    ...
    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(TimerViewModel::class.java)) {
            return TimerViewModel(application, sequence, timerList, firstInit, tts) as T
        }
